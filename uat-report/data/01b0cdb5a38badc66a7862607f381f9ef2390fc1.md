...
- Name: uat-07-material-filter.spec.ts >> UAT-07 | 素材筛选与列表 >> AC-002: 类型筛选下拉包含全部选项
- Location: tests/e2e/uat-07-material-filter.spec.ts:19:7
...
Locator: getByRole('combobox').nth(1).locator('option').filter({ hasText: '全部类型' })
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByRole('combobox').nth(1).locator('option').filter({ hasText: '全部类型' })
